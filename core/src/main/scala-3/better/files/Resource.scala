...
object Resource extends Resource {
  ...
  /** Look up class resource files.
    ...
    *   [[https://docs.oracle.com/javase/10/docs/api/java/lang/Class.html#getResource(java.lang.String) Class#getResource]]
    */
  inline def at[T]: Resource = ${ atStaticImpl[T] }

  private def atStaticImpl[T: Type](using qc: Quotes): Expr[Resource] = {
    import qc.reflect.*
    ...
    if (typeSymbolStr.startsWith("class ") || typeSymbolStr.startsWith("module class ")) {
      ...
      return '{
        new Resource {
          override def url(name: String) = Option(Class.forName(${ Expr(baseClass.fullName) }).getResource(name))
        }
      }
    } else {
  ...
  /** Look up own resource files.
    ...
    *   [[https://docs.oracle.com/javase/10/docs/api/java/lang/Class.html#getResource(java.lang.String) Class#getResource]]
    */
  inline def my: Resource = ${ myImpl }

  private def myImpl(using qc: Quotes): Expr[Resource] = {
    import qc.reflect.*
    ...
    while (callee != null && callee != Symbol.noSymbol) {
      ...
      if (callee.isClassDef) {
        return '{
          new Resource {
            override def url(name: String) = Option(Class.forName(${ Expr(callee.fullName) }).getResource(name))
          }
        }
      }
    }
