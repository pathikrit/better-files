...
private[files] final class ResourceMacros(val c: blackbox.Context) {
  ...
  def atDynamicImpl(clazz: Expr[Class[_]]): Expr[ResourceLoader] =
    reify {
      ...
      new ResourceLoader {
        override def url(name: String) = Option(clazz_.getResource(name))
      }
    }
  ...
  def myImpl: Expr[ResourceLoader] = {
    ...
    if (rtc.isEmpty) {
      ...
      c.abort(c.enclosingPosition, "can't use ‘my’ here, because this location doesn't correspond to a Java class file")
    }
